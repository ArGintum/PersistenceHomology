...
public:
    ...
    bool is_dominated(index_t s, index_t t, Graph& working_graph) {
        ...
        std::vector<index_t> common_vertices, working_common_list;
        std::set_intersection(Ns.first, Ns.second, Nt.first, Nt.second, std::back_inserter(common_vertices));

        if (common_vertices.size() == 1)
            return true;

        for (index_t v: common_vertices) {
            working_common_list.clear();

            auto Nv = boost::adjacent_vertices(v, working_graph);
            std::set_intersection(Nv.first, Nv.second, common_vertices.begin(), common_vertices.end(), std::back_inserter(working_common_list));

            if (working_common_list.size() + 1 == common_vertices.size())
                return true;
        }
    ...
    void push_neighboorhood(Graph& graph, Graph& neighbourhood, index_t s, index_t t) {
        ...
        std::vector<index_t> common_vertices;
        std::set_intersection(Ns.first, Ns.second, Nt.first, Nt.second, std::back_inserter(common_vertices));

        for(auto x: common_vertices) {
    ...
    void delete_dominated_edges(std::vector<diameter_index_t>& edges, std::vector<bool>& dominated) {
        ...
        for (auto e : edges) {
            ...
            Graph neighbourhood(n);

            push_neighboorhood(working_graph, neighbourhood, vertices_of_edge[0], vertices_of_edge[1]);
            ...
            while (j >= 0) {
                ...
                if (!boost::lookup_edge(vertices_of_edge[0], vertices_of_edge[1], neighbourhood).second) {
                    boost::remove_edge(vertices_of_edge[0], vertices_of_edge[1], working_graph);
                    --j;
                    continue;
                }

                if (!is_dominated(vertices_of_edge[0], vertices_of_edge[1], working_graph)) {
                    dominated[j] = false;
                    push_neighboorhood(working_graph, neighbourhood, vertices_of_edge[0], vertices_of_edge[1]);
                } else
                    boost::remove_edge(vertices_of_edge[0], vertices_of_edge[1], working_graph);
